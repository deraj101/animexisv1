...
export default function LoginScreen({ navigation }) {
  ...
  const sendCode = async (endpoint, body) => {
    ...
    if (res.data.success) {
      if (res.data.otpRequired === false || res.data.token) return res.data;
      setResendTimer(60);
      return res.data;
    }
  ...
  const finishAuth = async (data, fallbackEmail = email.trim().toLowerCase()) => {
    await signIn({
      ...
      profile_border: data.profile_border || null,
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      const auth = await sendCode("/api/auth/send-otp", { email: trimmed, password });
      if (auth?.token) {
        await finishAuth(auth, trimmed);
      } else if (auth) {
        setOtpPurpose("login");
        setStep("otp");
      }
  ...
  const handleActualRegister = async () => {
    ...
    setLoading(true); clearError();
    try {
      const auth = await sendCode("/api/auth/register", { email: trimmed, password, username: cleanUsername });
      if (auth?.token) {
        await finishAuth(auth, trimmed);
      } else if (auth) {
