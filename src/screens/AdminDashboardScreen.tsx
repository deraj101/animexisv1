...
export default function AdminDashboardScreen({ navigation }) {
  ...
  const handleToggleGlobalOtp = async () => {
    const nextOtpDisabled = !systemSettings?.otpDisabled;
    setSavingSettings(true);
    try {
      const cfg = { headers: await getAuthHeader() };
      const res = await API.post("/api/admin/settings", { otpDisabled: nextOtpDisabled }, { headers: cfg.headers });
      if (res.data.success) {
        setSystemSettings(res.data.settings);
        Alert.alert(
          nextOtpDisabled ? "OTP Turned Off" : "OTP Turned On",
          nextOtpDisabled
            ? "Users can now log in and create accounts without an email code."
            : "Users must verify with an email code again."
        );
      }
    } catch (err) {
      const msg = err.response?.data?.error || err.message;
