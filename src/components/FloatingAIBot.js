...
export default function FloatingAIBot({ getCurrentRouteName }) {
  ...
  const [greetingTitleText, setGreetingTitleText] = useState("");
  const [greetingBodyText, setGreetingBodyText] = useState("");
  ...
  useEffect(() => {
    if (!showGreeting || open) return undefined;
    ...
    setGreetingTitleText("");
    setGreetingBodyText("");

    [...GREETING_TITLE].forEach((_, index) => {
      schedule(() => setGreetingTitleText(GREETING_TITLE.slice(0, index + 1)), GREETING_TIMING.titleType * (index + 1));
    });
    ...
    [...GREETING_BODY].forEach((_, index) => {
      schedule(() => setGreetingBodyText(GREETING_BODY.slice(0, index + 1)), bodyStart + GREETING_TIMING.bodyType * (index + 1));
    });

    const eraseStart = bodyStart + GREETING_TIMING.bodyType * GREETING_BODY.length + GREETING_TIMING.hold;
    [...GREETING_BODY].forEach((_, index) => {
      schedule(() => setGreetingBodyText(GREETING_BODY.slice(0, GREETING_BODY.length - index - 1)), eraseStart + GREETING_TIMING.bodyErase * (index + 1));
    });

    const titleEraseStart = eraseStart + GREETING_TIMING.bodyErase * GREETING_BODY.length + GREETING_TIMING.gapBeforeTitleErase;
    [...GREETING_TITLE].forEach((_, index) => {
      schedule(() => setGreetingTitleText(GREETING_TITLE.slice(0, GREETING_TITLE.length - index - 1)), titleEraseStart + GREETING_TIMING.titleErase * (index + 1));
    });

    schedule(() => setShowGreeting(false), titleEraseStart + GREETING_TIMING.titleErase * GREETING_TITLE.length + GREETING_TIMING.hideDelay);
      ...
      >
        {showGreeting && !open && (
          <Pressable
            ...
            style={[styles.greetingBubble, greetingSideStyle, greetingVerticalStyle]}
          >
            <Text style={styles.greetingTitle}>
              {greetingTitleText}
              {greetingTitleText.length < GREETING_TITLE.length ? <Text style={styles.cursor}>|</Text> : null}
            </Text>
            <Text style={styles.greetingText}>
              {greetingBodyText}
              {greetingTitleText.length === GREETING_TITLE.length && greetingBodyText.length < GREETING_BODY.length ? (
